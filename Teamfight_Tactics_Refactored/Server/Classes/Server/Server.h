...
public:
    ...
    void notify(const SOCKET clientSocket, const std::string& message);
    friend void clientHandler(const SOCKET clientSocket, Server& server);
...
private:
    ...
    std::vector<std::map<SOCKET, std::string>> playerNames;
    std::vector<std::map<SOCKET, std::string>> battleMaps;
