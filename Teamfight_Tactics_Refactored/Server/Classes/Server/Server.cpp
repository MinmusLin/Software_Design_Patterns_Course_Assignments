...
Server::Server() : currentConnections(0) {
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        std::cerr << "Function WSAStartupError failed with error: " << WSAGetLastError() << std::endl;
        exit(SOCKET_ERROR);
    }
...
void Server::createAndBindSocket() {
    for (port = MIN_PORT_ADDRESS; port <= MAX_PORT_ADDRESS; port++) {
        ...
        if (bind(serverSocket, (struct sockaddr*)&server, sizeof(server)) == SOCKET_ERROR) {
            ...
            std::cout << "Server successfully created." << std::endl;
            char* ipv4 = inet_ntoa(*(struct in_addr*)*host->h_addr_list);
            std::cout << "IPv4: " << ipv4 << std::endl;
            std::cout << "Port: " << port << std::endl;
            break;
        }
...
void Server::attach(std::shared_ptr<Observer> observer) {
    observers.insert(observer);
}

void Server::detach(std::shared_ptr<Observer> observer) {
    observers.erase(observer);
}

void Server::notify(const SOCKET clientSocket, const std::string& message) {
    for (const auto& observer : observers) {
        observer->update(clientSocket, message);
    }
}
